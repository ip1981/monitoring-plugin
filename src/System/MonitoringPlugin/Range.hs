...
data Range n = Inner (Value n) (Value n) | Outer (Value n) (Value n)

-- | Test if a value belongs to the range.
within :: (Ord n, Num n) => n -> Range n -> Bool
within p (Inner v1 v2) = v1 <= v && v <= v2 where v = Fin p
within p (Outer v1 v2) = v < v1 || v2 < v where v = Fin p
...
showRange (Inner v1 v2) =
  if Fin 0 /= v1 then show v1 ++ ":" ++ show v2
                 else show v2
...
instance (Num n, Ord n, Read n) => Read (Range n) where
  ...
  readsPrec _ s = [ (r, "") | v1 <= v2 ]
    where
      outter = head s == '@'
      r = if outter then Outer v1 v2 else Inner v1 v2
      s' = if outter then tail s else s
      (s1, s2) = span (/= ':') s'
      (v1, v2) = if null s2 then ( Fin 0, read s1 )
                            else ( read s1, read $ tail s2 )


data Value n = NInf | Fin n | PInf
